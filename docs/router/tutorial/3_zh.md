...
定义布线层对可减少[交互式布线](/router/guide/19_zh.md#4)期间手动切换层的时间。配对布线层将层切换限制在层对成员之间。对于这个四层设计，明显的布线对由主元件层和次元件层组成。
...
布线角度设置决定了[交互式布线](/router/guide/19_zh.md#4)期间相邻导线段允许的角度（以度为单位）。
...
将布线和过孔网格设置为 8.33 密耳以适应设计的 8 密耳导线和 8 密耳间距要求：
...
在布线会话期间，平移设计可能很方便。SailWind Router 包含一个自动平移功能，可以跟随鼠标自动平移屏幕。

1. 在鼠标设置区域，选中**随鼠标移动平移显示**复选框以启用自动平移功能
...
在[交互式布线](/router/guide/19_zh.md#4)和编辑期间，SailWind Router 可以显示所有对象周围的防护带以指示间距边界。当鼠标接近对象时，会在附近对象周围显示可见边框，指示当前操作有效规则的间距边界。
