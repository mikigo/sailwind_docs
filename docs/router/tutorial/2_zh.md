...
4. [创建网络类](/router/guide/11_zh.md#21)规则集
...
在 SailWind Router 中，您可以为设计[规则层次结构](/router/guide/11_zh.md#3)的每个级别定义 SailWind Layout 的间距、布线、扇出和焊盘入口约束。
...
> !tip] 由于[项目资源管理器中看不到网络类分支，请将选择拖到项目资源管理器顶部以强制列表滚动。这使您可以将选择放在网络类分支中。分支展开后，您可以看到已[创建网络类](/router/guide/11_zh.md#21) Class1。
...
条件设计规则是指当对象满足某些条件时生效的规则，例如当网络靠近另一网络或其他设计对象布线时。条件规则最常用于两个网络紧密布线时，以避免对电路产生不利影响(如串扰)。例如，当交流电直接连接到 PCB 时，美国保险商实验室(UL)要求隔离初级、次级和接地网络。您可以在设计[规则层次结构](/router/guide/11_zh.md#3)的大多数组件之间分配条件规则。条件规则可以存在于网络之间、网络和类之间、类和类之间、网络和层之间等。

**[创建条件规则](/router/guide/11_zh.md#24)**

您可以在项目资源管理器中创建或修改设计对象。在本练习中，您将在项目资源管理器中[创建条件规则](/router/guide/11_zh.md#24)。
...
5. 双击展开条件规则分支。您可以看到已[创建条件规则](/router/guide/11_zh.md#24)+5V : +12V (所有层)。
