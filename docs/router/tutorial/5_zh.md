...
SailWind Router 提供全面的[交互式布线](#1)命令选择。
...
[交互式布线](#1)器是 SailWind Router 中的核心导线编辑功能。导线编辑器中的许多操作类似于 SailWind Layout 中的操作，例如创建多边形和线条项目。这可以让您将相同的技能应用到许多领域，从而最小化 SailWind Router 的学习曲线。
...
4. 右键点击并选择 **Interactive Route**。新的导线段开始附着到鼠标

> [!tip] 此时 DRC 模式关闭。新导线段不禁止与其他对象短路。如果启用了 DRC，此时可通过输入 **DRC** 然后按 **Enter** 禁用它。

5. 移动鼠标并注意连接的一端如何附着到新导线段的末端，而另一端保持连接到连接的端点。这有助于您在布线时确定方向

6. 要添加拐角，请点击

7. 要删除拐角，请按 **Backspace**

在具有多个连接的网络中，连接的端点会自动捕捉到网络上最近的管脚。通过将鼠标移向电路板的左下角来尝试重新连接。一旦导线段的端点更接近 U7，它就会从当前端点分离并捕捉到 U7。将鼠标移向左上方，导线段会重新捕捉回来。此功能让您在布线时动态重新连接导线，无需手动重新排序连接。

> [!tip] 在本练习的任何时候，您都可以通过按 Esc 退出布线命令操作。您也可以点击 **Undo** 按钮撤销任何操作。

**[更改布线角度模式](/router/guide/20_zh.md#2)**

新线段被限制为从导线段原点开始的 90 度增量，因为当前导线角度模式为正交。您可以通过点击快捷菜单上的命令在布线期间更改导线角度模式。

当导线附着到鼠标时，右键点击，指向 **Routing Angle**，然后点击 **Diagonal**。更改角度模式后，移动鼠标。注意新线段现在如何被限制为 45 度增量。

**添加过孔和更改层**

更改层与添加拐角的方式相同，只是在点击时按 Shift。您可以在当前鼠标位置或布线时的最后一个拐角位置启动层变更。

**在当前鼠标位置启动层变更：**

当新导线段附着到鼠标时，在要更改层的位置使用 **Shift+点击**。在点击位置添加一个新过孔。布线层对的第二层现在是当前层。

**替代方法：** 当新导线段附着到鼠标时，点击添加拐角。右键点击，指向 Layers，然后从列表中选择一个层。如果不先添加拐角，附着到鼠标的导线段将更改层而不是添加过孔和更改层。

**在最后一个拐角位置启动层变更：**

1. 点击添加新拐角

2. 将鼠标从新拐角移开，右键点击并点击 **Layer Toggle**。在最后添加的拐角处添加一个过孔
...
**[更改导线宽度](/router/guide/20_zh.md#7)**

新导线段以网络指定的默认推荐导线宽度布线。如果为网络指定了不同的最小值和/或最大值，您可以通过点击快捷菜单上的命令在布线期间[更改导线宽度](/router/guide/20_zh.md#7)。

1. 当导线附着到鼠标时，右键点击，指向 **Width**，然后从列表中选择新宽度。选项包括为网络指定的最小、推荐和最大宽度值

2. 如果所需宽度值不在列表中，点击 **Set** 并输入介于网络指定的最小值和最大值之间的值。导线宽度将更改为该值，并且这个新值将被添加到列表中

[更改导线宽度](/router/guide/20_zh.md#7)后，移动鼠标。注意新线段现在如何以新指定的宽度添加。
...
当导线段附着到鼠标时，从其起点到终点定义导线模式，并在完成符号出现时点击鼠标左键。新导线完成，在许多情况下不需要平滑或清理导线。

**使用 Complete 命令远程完成导线**

当新导线段附着到鼠标时，右键点击并点击 **Complete**。您也可以双击鼠标左键完成导线。新导线从其起点到目的地完成。

练习将 24MHz 网络的短连接布线至完成。

> [!tip] 在本练习的任何时候，您都可以通过按 Esc 退出布线命令操作。您也可以点击标准工具栏上的 Undo 按钮撤销任何操作。

**在[交互式布线](#1)操作期间添加带圆弧的导线**

1. 通过选择未布线部分并按 **F3** 开始[交互式布线](#1)

2. 当新导线段附着到鼠标时，右键点击，指向 **Arc**，然后点击 **Add Arc**

**替代方法：按 Alt+A**

3. 点击要结束圆弧的点。SailWind Router 返回到正常的[交互式布线](#1)模式，允许您完成导线布线

> !tip] 如果要指定圆弧的确切半径，通过输入 RAD 并输入要使用的半径来设置半径。然后在[[交互式布线](#1)时，右键点击，指向 Arc，然后点击 Add Arc of Radius。
...
对于具有嵌入式平面和表面安装元件的典型 PCB，[平面网络布线](#2)仅限于从焊盘布出一小段并用过孔终止以提供与内部平面的接触。

在开始[平面网络布线](#2)之前，您需要更新平面网络的可见性。
...
1. 开始[交互式布线](#1)，当导线段附着到鼠标时，右键点击，指向 **End Via Mode**，然后点击 **End Via**。勾选标记指示当前模式
...
> [!tip] 在 DRC Explain 模式下布线会在鼠标处创建一个工具提示解释违规。要启用 DRC Explain 模式，在 Design Rule Checking 对话框中，在 Error Response 列表中选择 Explain。

## 动态布线 \{#5}

强大的动态布线功能在 SailWind Router 中提供了前所未有的[交互式布线](#1)控制水平。

动态布线是另一个强大的[交互式布线](#1)功能。您不需要指示每个导线拐角，只需开始布线并朝希望导线流动的方向移动鼠标。导线拐角在您移动鼠标时动态添加。
...
3. 选择靠近 U7（视图底部）的 24MHz 网络，右键点击并点击 **Interactive Route**。一条导线动态附着到鼠标

**替代方法：按 F3**

4. 在电路板上垂直移动鼠标，朝连接目的地的大致方向。注意使用动态[交互式布线](#1)如何自动选择绕过障碍物的路径并创建导线模式

5. 通过移动附着导线的鼠标绕过障碍物来尝试动态。将鼠标移近连接目的地并双击。注意动态如何自动完成导线并平滑模式

> !tip] 在本练习的任何时候，您都可以通过按 Esc 退出[[交互式布线](#1)。您也可以点击标准工具栏上的 Undo 按钮撤销任何操作。
...
5. 将鼠标向左移动并按 **Shift+点击** 在 J1 的禁布区边框内添加过孔。注意插入过孔失败
