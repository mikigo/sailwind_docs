...
4. 保存自定义颜色方案。您可以在常用颜色方案之间切换以处理设计

🏃‍♂️‍**步骤**

1. 点击 **工具 > 选项** 菜单项

2. 在"选项"对话框中，点击鼠标"颜色"类别
...
您可以保存设计工作中常用的显示颜色设置。参见"保存配色 方案"。

您可以使用无模命令切换显示管脚编号和网络名称。参见"表 19"。
...
要使用先前保存的配置，从"颜色方案名称"列表中选择配置名称。

## 高亮与取消高亮设计对象 \{#4}

您可以高亮或取消高亮设计中的对象，以区别于其他设计对象。在处理密集设计时，高亮有助于视觉定位对象。

您可以使用"[选项对话框，全局类别，常规子类别](/router/guide/24_zh.md#30)"中的"通过以下方式区分高亮对象"列表设置高亮显示方式。
...
> [!tip] 您也可以使用 HG、HD 和HH 快捷键高亮对象。参见"表 24"。
...
> [!tip] 您也可以使用 HG、HD 和HH 快捷键执行此操作。参见"表 24"。
...
| Page Up    | 以鼠标位置为中心放大                                                                                                                     |
| Page Down  | 以鼠标位置为中心缩小                                                                                                                     |
| Ins        | 以当前鼠标位置为中心显示视图，不进行缩放                                                                                                 |
...
如果需要，您可以按住 Alt 键并滚动鼠标滚轮在设计层之间滚动。此功能可用于在[交互式布线时预览层](#7-1)以及[显示单个电气层](#7-2)。
...
1. 在[交互式布线](/router/guide/19_zh.md#4)时，按住 Alt 键并向前或向后滚动滚轮按钮

2. 执行以下操作之一：

   - 要在当前鼠标位置过渡层，按住 Shift 并点击。鼠标位置将出现一个过孔，允许您从该点在预览的层上继续布线

   - 要移动进行中的导线（从最后一个拐角位置），按住 Ctrl 并点击鼠标中键。最后一个拐角位置将出现一个过孔，并将导线移动到您预览的层

### 显示单个电气层 \{#7-2}

您可以循环[显示单个电气层](#7-2)集。切换到相邻电气层时会跳过非电气层，并使所有其他层不可见。
