...
- **导航窗口** 该窗口在导航设计时提供工作区的替代视图：
	...
	- 在[交互式布线](/router/guide/19_zh.md#4)期间监控并显示任何长度控制网络的布线长度
...
您仍然可以使用自动化；但是，大多数自动化方法不会返回任何有价值的结果，也不会在数据库中做出任何更改。
 ...
 [启用同步模式](#3-1)   [在同步模式下发送设计到 SailWind Router](#3-2)   [切换到 SailWind Layout](#3-3)   同步模式中不支持的操作   [设计不同步时重新同步](#3-5)   [退出同步模式](#3-6) 
...
1. 按照"[启用同步模式](#3-1)"所述[启用同步模式](#3-1)。

2. 使用以下方法之一切换到 SailWind Router：

	- 在标准工具栏上，点击**布线**按钮。

	- 点击**工具 > SailWind Router**，在 SailWind Router 链接对话框中选择一个操作并点击**继续**。
