...
- 原理图符号，在库中称为 [CAE 封装](/logic/guide/7_zh.md#7)
- 元件封装，在库中称为 PCB 封装
- 电气参数，如管脚编号和门分配
...
|----------------------|--------------------------------------------|
| 元件类型名称:        | 7404                                       |
| [CAE 封装](/logic/guide/7_zh.md#7):             | INV(反相器)                                |
...
添加到原理图的每个元件都必须有一个 [CAE 封装](/logic/guide/7_zh.md#7)来图形化表示元件的逻辑功能。

虽然有些元件完全用单个 [CAE 封装](/logic/guide/7_zh.md#7)表示，但其他元件由同一 [CAE 封装](/logic/guide/7_zh.md#7)的多个实例组成，如 7404 元件(上述多门反相器)。

[CAE 封装](/logic/guide/7_zh.md#7)由连接到终端周围的管脚封装组成，这些终端围绕图形符号间隔放置。

该实体表示设备全部或部分。终端可以分配管脚名称来表示管脚的逻辑功能，如 IN、OUT 或 VSS。所有 [CAE 封装](/logic/guide/7_zh.md#7)的元素都在元件编辑器中创建。



## 创建 CAE 封装 \{#2}
在本练习中，您将使用 [CAE 封装](/logic/guide/7_zh.md#7)向导和其他功能为 87C256 元件类型[创建 CAE 封装](#2)，如下方图形所示。
...
1. 工具菜单 > 元件编辑器
2. 在元件编辑器工具栏上点击**编辑图形**按钮，然后点击**确定**确认创建具有默认名称的新 [CAE 封装](/logic/guide/7_zh.md#7)

进入 [CAE 封装](/logic/guide/7_zh.md#7)编辑器后，会出现几个文本标签和一个 [CAE 封装](/logic/guide/7_zh.md#7)原点标记。

每个标签都是 [CAE 封装](/logic/guide/7_zh.md#7)相关文本对象的占位符。这些标签在 [CAE 封装](/logic/guide/7_zh.md#7)上的位置决定了 [CAE 封装](/logic/guide/7_zh.md#7)文本对象在原理图中出现的位置。

原点标记作为移动或定位 [CAE 封装](/logic/guide/7_zh.md#7)的原点。
...
**使用 [CAE 封装](/logic/guide/7_zh.md#7)向导[创建 CAE 封装](#2)**

1. 封装编辑器工具栏按钮 > [CAE 封装](/logic/guide/7_zh.md#7)向导按钮
...
您现在有了 [CAE 封装](/logic/guide/7_zh.md#7)的开始部分。此过程创建了一个具有 19 个终端(管脚封装末端的小 X)的 [CAE 封装](/logic/guide/7_zh.md#7)，每个终端有一个 PIN 类型的管脚封装。

**参见：** *SailWind Logic 帮助*了解终端与管脚封装之间的关系

**添加新终端**

此时，[CAE 封装](/logic/guide/7_zh.md#7)尚未完成。您必须添加输出管脚并修改输入管脚上的部分符号。

1. 添加终端按钮**。**
2. 从 Pins 列表点击**PIN**封装，点击**确定**
3. 新终端附加到鼠标后，右键点击**X 镜像**，在 X 轴或水平方向镜像终端
4. 将终端定位在符号右侧 X**1000**，Y**1900**处，点击放置终端

**使用[分布和重复](/logic/guide/9_zh.md#9)添加新终端**

添加终端后，SailWind Logic 保持在添加终端模式；下一个新终端将附加到鼠标。使用[分布和重复](/logic/guide/9_zh.md#9)功能可以快速添加多个终端，而不是像上面那样逐个添加。

1. 下一个新终端附加到鼠标后，右键点击**[分布和重复](/logic/guide/9_zh.md#9)**
...
添加输出管脚后，使用其他终端命令修改输入管脚排列并完成 [CAE 封装](/logic/guide/7_zh.md#7)。输入管脚包括 15 个地址输入、3 个控制输入和 1 个额外输入管脚。其中两个控制管脚的管脚封装需要修改，必须移除额外终端。
...
1. [设置管脚名称](/logic/guide/7_zh.md#7-10)按钮**。**
...
要完成封装定义，以与分配管脚名称相同的方式为终端[设置管脚编号](/logic/guide/7_zh.md#7-9)。

1. 在封装编辑工具栏，点击**[设置管脚编号](/logic/guide/7_zh.md#7-9)**按钮
2. 在[设置管脚编号](/logic/guide/7_zh.md#7-9)对话框，接受默认值并点击确定
...
如果分配了错误的管脚编号，再次点击[设置管脚编号](/logic/guide/7_zh.md#7-9)按钮，输入新编号并点击终端。

**命名 [CAE 封装](/logic/guide/7_zh.md#7)和元件类型**

1. 文件菜单 > 返回元件
2. 点击**是**保留门更改
3. 在文件菜单点击**另存为**
4. 在保存元件和门封装对话框中，双击元件名称框，输入**87C256**
5. 在门封装名称框中，双击 [CAE 封装](/logic/guide/7_zh.md#7) 1 列下的**NEW\_PART**，输入**87C256**
6. 将库切换为**\Libraries\preview**
7. 点击**确定**。如果 [CAE 封装](/logic/guide/7_zh.md#7)已存在，确认覆盖
8. 点击**确定**确认没有[分配 PCB 封装](/logic/guide/7_zh.md#8-4)的警告。如果元件类型已存在，确认覆盖

## 为 CAE 封装创建元件类型 \{#3}
现在您将为 87C256 元件类型添加电气属性并[分配 PCB 封装](/logic/guide/7_zh.md#8-4)。
...
**[分配 PCB 封装](/logic/guide/7_zh.md#8-4)**

创建元件类型的下一步是分配 87C256 [CAE 封装](/logic/guide/7_zh.md#7)并分配 28 管脚 PCB 封装。
...
7. 完成所有属性添加后，点击**确定**完成电气属性分配过程并关闭[元件信息对话框](/logic/guide/24_zh.md#85)
...
您可以从[库管理器对话框](/logic/guide/24_zh.md#64)中编辑库中的现有元件类型。

1. 文件菜单 > 库
2. 在库列表中，切换为\Libraries\preview 库
3. 点击**元件**按钮
4. 在元件类型列表中，点击**87C256**。注意预览窗口中的图像。它应该是与元件类型关联的 [CAE 封装](/logic/guide/7_zh.md#7)
5. 点击**编辑**将元件带入元件编辑器。关闭库管理器。您现在可以编辑元件类型、其 [CAE 封装](/logic/guide/7_zh.md#7)和电气属性
6. 在文件菜单点击**退出元件编辑器**
7. 不要保存文件副本
