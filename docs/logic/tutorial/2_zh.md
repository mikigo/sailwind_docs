...
移动鼠标时，其相对于原点的绝对 X,Y 坐标显示在屏幕右下角状态栏。本教程中将参考此坐标数据。

1. 将鼠标置于原点上方，注意状态栏显示的 0,0 读数
2. 移动鼠标观察 X,Y 坐标变化
...
对于双键鼠标，缩放图标可启用/禁用缩放模式。也可通过视图菜单点击缩放启用。缩放模式下鼠标变为放大镜。三键鼠标始终可用中键进行缩放。

> [!TIP]
>
> 某些厂商为中键指定特定功能，可能需在控制面板中更改鼠标功能才能在 SailWind Logic 中使用这些特性。

通过将鼠标置于区域中心并向特定方向拖动可放大/缩小。

[平移和缩放](#3)功能也可通过视图菜单命令、数字键盘和滚动条实现。详见 SailWind Logic 帮助获取更多平移缩放功能信息。
...
2. 放大
   ...
   b. 向上拖动鼠标（远离身体方向），动态矩形随鼠标移动
   
   c. 当矩形包围要放大的区域时释放鼠标完成操作
   
3. 缩小

   重复步骤 2，但向下拖动鼠标（朝向身体方向），静态矩形表示当前工作区视图，动态矩形表示新视图
...
将鼠标移向要平移的方向并点击中键，新视图将使鼠标位置成为视图中心。

## 选择对象 \{#4}

选择元件时，将鼠标置于元件边框上点击。点击元件内部不能确保选中，特别是较大元件时。

### 选择过滤器 \{#4-1}

由于 [CAE 封装](/logic/guide/7_zh.md#7)（原理图符号）由多种对象组成，且原理图上还有许多其他对象，[选择过滤器](#4-1)便于选择单一类型对象。

1. 调整视图显示 U2 和 U4 元件周围区域（如下图）

2. 右键点击过滤器
   
   或按 Ctrl+Alt+F 打开[选择过滤器](#4-1)
   
3. 在[选择过滤器](#4-1)中点击任意，注意元件和网络未被选中而其他都被选中
...
7. 右键点击选择连接，点击选中同一连接段，所有连接至连接点的线段都被选中。右键点击选择网络实例，该网络实例所有连接段都被选中。
   ...
   > [!TIP]
   >
   > - 使用过滤器预设可快速更改[选择过滤器](#4-1)设置。设计中未选中任何对象时，右键点击选择命令之一
   > - 选择工具栏也包含[选择过滤器](#4-1)设置按钮
