...
使用文件操作创建并保存新的原理图文件。您还可以打开和保存现有设计、导入和导出其他文件格式的文件以及归档原理图。
...
2. 如果点击“**否**”，工作区中将显示一张代表新原理图鼠标空白图页。

   如果点击“**是**”，且当前原理图未命名，则会出现“[保存原理图文件](#2)”对话框。
   ...
   >
   > b. **右键**点击，然后点击**新建 > SailWind Logic Schematic** 。此操作会在当前文件夹中创建一个新的设计文件。
   >
...
限制：其他用户打开的文件将被锁定，无法进行任何编辑。请参阅[打开已在使用的文件](/logic/guide/3_zh.md#4)。

当前打开的设计名称显示在对话框底部的“文件名”文本框中。点击“保存”即可保存文件。如果文件名已存在，则会提示您是否覆盖现有文件。新设计将显示默认文件名。

您还可以通过点击标准工具栏上的“保存”按钮来保存文件。
...
其他用户打开的文件将被锁定，无法进行任何编辑。请参阅鼠标“[打开已在使用的文件](/logic/guide/3_zh.md#4)”。

🏃‍♂️‍**步骤**

1. 在工具栏上，点击**打开**按钮。

   “文件打开”对话框将显示默认 SailWind Projects 文件夹中包含的原理图文件。列表根据“文件类型”设置显示文件。

2. 从“文件类型”下拉列表中选择“**SailWind Logic File (\*.sch)**”。

3. 选择一个原理图文件，然后点击“**打开**”，将设计加载到 SailWind Logic 中。

   或者，您也可以通过将文件从 Windows 资源管理器**拖放**到 SailWind Logic 窗口中来打开文件。
...
| 上的 OLE 导入/导出 (*.ole) | SailWind Logic 允许您使用“编辑/插入新对象”将其他应用程序中的文件作为 OLE 对象嵌入到原理图中。一旦您的设计中包含了 OLE 对象，您就可以使用“文件导出”将该对象作为单个项目导出到 .ole 文件（参见导出文件）。然后，您可以将 .o/e 文件导入到其他 SailWind Logic 原理图中。 |
| ECO 文件 (*eco) | 类似于 PADS 格式的 ASCll。每种类型的数据都以标题行开头，标题行中包含一个关键字，关键字周围用星号 (*) 括起来。 |
| （.asc）文件 | SailWind Layout规则。 |

表 8. 作为新原理图打开的文件类型

| 文件类型 | 描述 |
| --- | --- |
| CAD .csa 文件 | CADSTAR 归档文件 (ASCII) |
...
1. 点击主页 > 导入。
2. 在显示的提示中，点击“是”在导入文件之前保存现有设计，或点击“否”导入文件而不保存更改。
3. 从“文件类型”下拉列表中，选择要导入的文件类型。
4. 选择要导入的文件，然后点击“打开”。状态栏中会报告导入进度，输出窗口中会显示日志消息和日志文件链接。
...
5. 为导出的文件选择 SailWind Logic 输出版本。
   ...
   - 在之前的 PADS 版本中，任何带有 DIE 或 FLP 系列标识的普通元件均被视为裸片元件或倒装芯片。更多信息，请参阅 [OLE 对象导入和导出](#4-4)。
...
2. 键入输出文件的名称，然后点击“保存”。
   ...
   >[!TIP]
   >文件名和路径出现在 [ASCII 输出对话框](/logic/guide/24_zh.md#15)的底部。

3. 在 [ASCII 输出对话框](/logic/guide/24_zh.md#15)中，选择“选择要输出的部分”区域中要出现在 ASCII 文件中的项目。

   您可以点击“全选”以选择所有项目，或点击“取消全选”以清除所有选择。
...
您可以创建一个文件夹、一个 PDF 或一个 .zip 文件，其中包含所有原理图文件和支持文件。这包括原理图本身、设计文件、库以及您需要的任何其他文件或文件夹。您可以选择要归档的内容；所有字段均为可选。

🏃‍♂️‍**步骤**

1. 打开您想要归档的原理图。
2. 点击**主页 > 归档**。
3. 在“归档器”对话框中，选择要归档的文件和文件夹。
4. 点击“**确定**”。

👀‍**结果**

将创建一个归档文件夹，其中包含设计或原理图文件、库以及您指定的任何其他文件和文件夹。

如果您选择压缩文件，则 `.zip` 文件是此文件夹中的唯一文件。

如果您选择创建 PDF，则使用原理图名称创建文件并将其放置在归档文件夹中。
...
其中 YYYY 代表年份，MM 代表月份，DD 代表日期，HH 代表小时（采用 24 小时制），MM 代表分钟，SS 代表文件创建时间的秒数。该文件包含与归档文件夹相同的文件夹结构。
