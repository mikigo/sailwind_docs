...
使用 SailWind Logic，您可以创建和修改库、设置可搜索性、[管理库属性](#5)、[导入和导出库](#6)、报告内容以及使用搜索参数。
...
| .ld | Logic_—— 原理图元件的图形表示，例如或非门。此部分仅用作元件清单读取器。使用 SailWind Logic 创建和修改 Logic 封装（有时称为 [CAE 封装](/logic/guide/7_zh.md#7)）。 |
| .In | 线——您可以存储在库中的一般图形数据，例如公司徽标，以便在任何设计文件中使用。 |

有关创建 PCB 封装的信息，请参阅 SailWind Layout指南中的“创建新封装”。

## 创建库 \{#1}

要将新库添加到库列表，请先创建一个新的空库，然后向其中填充内容。添加后，您可以将其重新放置在库列表中按搜索顺序排列的首选位置。

有关详细信息，请参阅[将库添加到库列表](#4-1)。

🏃‍♂️‍**步骤**

1. 点击主页 > 库。
2. 在“库管理器”对话框中，点击“创建新库”。
3. 在新建库对话框中，指定文件夹和库文件名，然后点击保存。
4. 点击“关闭”。

👀‍**结果**

您的库将被添加到库列表的底部，同时也是库检索顺序中的最后一个库。要将您的库在库列表和检索顺序中上移，请参阅[设置库列表顺序](#4-4)。

## 显示库中的项目 \{#2}

您可以在“库管理器”对话框中显示库中现有的项目。您可以选择查看 PCB 封装、元件类型、绘图项目或逻辑 [CAE 封装](/logic/guide/7_zh.md#7)。
...
3. 点击以下按钮之一可[显示库中的项目](#2)类别：
   ...
   - 逻辑 — [CAE 封装](/logic/guide/7_zh.md#7)（示意图符号）

4. 要过滤列表，请在筛选条件框中键入上的通配符或表达式，然后点击应用。
...
库项目名称显示在 PCB 封装、元件类型、线项目或 [CAE 封装](/logic/guide/7_zh.md#7)列表中。（列表名称会根据您选择的筛选条件而变化。）预览窗口会显示库对象的图形。

> [!INFO]
>
> 由于库元件没有视觉表示，预览窗口显示与该元件关联的第一个 [CAE 封装](/logic/guide/7_zh.md#7)。

## 修改库数据 \{#3}

您可以通过添加、删除、复制、编辑或传输数据来[修改库数据](#3)，以满足特定设计的要求。
...
2. 在[库管理器对话框](/logic/guide/24_zh.md#64)中，从库列表中选择一个库。
...
4. 点击新建按钮。
   ...
   - 元件 — 您将进入元件编辑器。点击**“编辑电参数”**按钮。系统将显示“元件信息”对话框，您可以在其中定义新元件。另请参阅修改元件的电气信息。

   - 直线 - “新建”和“编辑”不可用。所有 2D 绘图项目（例如，绘图格式和标题栏）都会显示，您只能复制或删除它们。没有专门的库直线编辑器。请使用绘图工具创建或编辑直线并将其保存到库中。

     >  [!TIP]
     > 
     > 要将新的绘图项目添加到库中，请在上创建并合并 SailWind Logic 中的项目，然后点击鼠标右键，再点击“保存到库”。

     另请参阅[创建 2D 线项目](/logic/guide/12_zh.md#1)、将绘图项目添加到库。

   - 逻辑 — 您将进入 [CAE 封装](/logic/guide/7_zh.md#7)编辑器。另请参阅[创建新的 CAE 封装](/logic/guide/7_zh.md#7-4)。
...
5. 在“PCB 封装”、“元件类型”、“行项目”或“[CAE 封装](/logic/guide/7_zh.md#7)”列表中选择一个或多个项目。（列表名称会根据您选择的筛选条件而变化。）

   > [!TIP]
   > 按住 Ctrl 键并点击可选择多个不连续的项目。按住 Shift 键 $^+$ 并点击或拖动光标可选择一系列项目。
...
6. 在打开的对话框中，选择另一个库来接收该项目的副本，和/或键入新的项目名称，然后点击“确定”。

   - 封装 — 打开将 PCB 封装保存到库对话框
   - 元件 — 打开[将元件类型保存到库对话框](/logic/guide/24_zh.md#118)
   - 线 — 打开[将绘图项目保存到库对话框](/logic/guide/24_zh.md#115)
   - 逻辑 — 打开[将 CAE 封装保存到库对话框](/logic/guide/24_zh.md#113)


### 编辑库中的项目 \{#3-4}

由于库内容经常变化，您可以使用库管理器[编辑库中的项目](#3-4)。选择要编辑的项目，进行所需的编辑，然后将编辑保存到当前库项目或将该项目重命名为新的库对象。
...
5. 在列表中选择一个项目并点击“编辑”。
   ...
   - 线 - 不可用。所有 2D 绘图项目（例如，绘图格式、标题栏等）都会显示，您只能复制或删除它们。没有专门的库线编辑器。使用绘图工具创建或编辑线并将其保存到库中。
     ...
     > [!TIP]
     > 
     > 要将新的绘图项目添加到库中，创建并合并 SailWind Logic 中的项目，然后点击鼠标右键，再点击“保存到库”。

   - 逻辑 — 打开包含选定封装的 [CAE 封装](/logic/guide/7_zh.md#7)编辑器。请参阅[创建新的 CAE 封装](/logic/guide/7_zh.md#7-4)。

### 删除库中的所有项目 \{#3-5}

当您选择[删除库中的所有项目](#3-5)时，软件会用一个同名的新空库替换现有库。这样，软件可以确保您的库中没有剩余的项目。
...
1. 点击主页 > 库。
2. 点击“创建新库”。将显示“新建库”对话框。
3. 选择要删除内容的库文件，点击“保存”，然后点击“是”。
提示。
4. 点击“关闭”。

### 传输库数据 \{#3-6}

有时可能需要将库对象从一个库转移到另一个库。当您想将某个设计的特定库归档，或者为新设计创建包含特定内容组合的库时，这可能会很有用。

> [!TIP]
> 
> 要复制单个项目，请参阅[复制库项目](#3-3)。


🏃‍♂️‍**步骤**

1. 按照上的步骤从库中[导出库数据](#6-2)。
2. 按照 SailWind Layout指南的[导入库数据](#6-1)主题的步骤将库数据导入另一个库。

## 设置库可用性和搜索选项 \{#4}

指定设计可用的库、[库搜索顺序](#9)和其他搜索相关选项。

操作会影响[库管理器对话框](/logic/guide/24_zh.md#64)中库列表的内容。

### 将库添加到库列表 \{#4-1}

您可以[将库添加到库列表](#4-1)中，以使其内容可用。库必须已在“库管理器”中列出，才能搜索并在设计中使用其元件、封装或线项。

🏃‍♂️‍**步骤**

1. 点击主页 > 库，然后点击管理库列表按钮。

2. 在[库列表对话框](/logic/guide/24_zh.md#63)中，点击添加。
...
1. 点击主页 > 库，然后点击管理库列表按钮。
2. 在[库列表对话框](/logic/guide/24_zh.md#63)中，从库列表中选择一个或多个库，然后点击移除。
...
- 使用各种对话框搜索库项目从原理图
- 导入网表
- [从库中更新](/logic/guide/10_zh.md#9)您的设计
- 使用原理图上的新元件注释您的设计

您可以使用“[库列表对话框](/logic/guide/24_zh.md#63)”更改库的搜索顺序。
...
1. 点击主页 > 库，然后点击管理库列表按钮。
2. 从[库列表对话框](/logic/guide/24_zh.md#63)中的库列表中选择库，然后根据需要点击向上或向下。
...
2. 选择所有四个库文件，**右键**点击并点击“属性”。
3. 在“属性”对话框中，点击“常规”选项卡，然后选中“只读”复选框。
4. 点击“确定”。

👀‍**结果**

直到您重新打开该对话框时，[库列表对话框](/logic/guide/24_zh.md#63)中的库只读复选框才会更新。

### 与 SailWind Layout同步 \{#4-8}
...
1. 点击主页 > 库，然后点击管理库列表按钮。
2. 在[库列表对话框](/logic/guide/24_zh.md#63)中，选中[与 SailWind Layout同步](#4-8)复选框。

## 管理库属性 \{#5}

使用“[管理库属性](#5)”对话框可以逐个库地管理属性。您可以添加、删除和重命名单个库或所有库中所有元件或封装的属性。您还可以显示库中的所有属性，无论这些属性是应用于所有项目还是应用于单个项目。

> [!TIP]
> 
> 此对话框不管理当前设计中的属性。使用“管理原理图属性”对话框可管理打开的设计文件中的属性。

向多个库项目添加属性 从库项目中删除属性 [重命名库项目的属性](#5-3)
...
1. 点击主页 > 库，然后在[库管理器对话框](/logic/guide/24_zh.md#64)中点击属性管理器按钮。
2. 在“[管理库属性](#5)”对话框的“选择库”列表中，选择单个库或列表顶部的“（所有库）”项。
3. 在项目类型列表中，选择是否将新属性应用于所有项目，或者仅应用于元件类型或 PCB 封装。
4. 点击“添加属性”。 此时将显示“将新属性添加到库”对话框。
5. 对于属性名称，请执行以下操作之一：
   - 在框中键入属性名称。
   - 点击浏览库属性以在所有库中搜索现有的属性名称。
6. （可选）在属性值框中键入一个值。
7. 点击“确定”。
...
新属性已添加。请在“封装属性”对话框（针对 PCB 封装）或位于鼠标“元件信息”对话框的“属性”选项卡（针对元件类型）中检查新属性。
...
4. 在“库中的属性”列表中，选择一个或多个要删除的属性，然后点击“删除属性”。
5. 在出现的提示中，点击“确定”。
6. 点击“关闭”。

👀‍**结果**

您的属性已被删除。请在“封装属性”对话框（针对 PCB 封装）或位于鼠标“元件信息”对话框的“属性”选项卡（针对元件类型）中检查已删除的属性。
...
1. 点击主页 > 库，然后在[库管理器对话框](/logic/guide/24_zh.md#64)中点击属性管理器按钮。
2. 在“[管理库属性](#5)”对话框的“选择库”列表中，选择单个库或列表顶部的“（所有库）”项。
3. 在项目类型列表中，选择是否从所有项目、元件类型或 PCB 封装中[重命名属性](/logic/guide/10_zh.md#18-2)。
4. 在“库中的属性”列表中，选择一个或多个要重命名的属性，然后点击“添加”。
5. 双击“新名称”单元格，键入名称，然后点击“[重命名属性](/logic/guide/10_zh.md#18-2)”。
...
您的属性已重命名。请在“封装属性”对话框（针对 PCB 封装）或位于鼠标“元件信息”对话框的“属性”选项卡（针对元件类型）中检查重命名的属性。

## 导入和导出库 \{#6}

使用[库管理器对话框](/logic/guide/24_zh.md#64)以 ASCII 格式导入或[导出库数据](#6-2)。

### 导入库数据 \{#6-1}

您可以从先前导出的库 ASCII 文件中[导入库数据](#6-1)。这为从其他设计或其他设计师的库中[导入库数据](#6-1)提供了一种便捷的方法。

> [!TIP]
> 
> 从 PADS 9.0 开始，裸片元件和倒装芯片不再通过其系列名称（DIE 或 FLP）来识别，而是通过“元件信息”对话框“常规”选项卡中的“特殊用途”设置来识别。导入由旧版 PADS 创建的 ASCII 文件时，这些“特殊用途”设置将自动应用于逻辑系列为 DIE 或 FLP 的元件。元件的系列名称保持不变。

🏃‍♂️‍**步骤**

1. 点击主页 > 库。
2. 在[库管理器对话框](/logic/guide/24_zh.md#64)中，在库列表中，选择要接收库数据的库。
3. 要导入四种文件类型之一，您必须选择匹配的筛选条件：
   - 如果文件类型为.c，请选择“逻辑”筛选条件。此 ASCII 文件包含 [CAE 封装](/logic/guide/7_zh.md#7)（逻辑符号）。
...
1. 点击主页 > 库。
2. 在[库管理器对话框](/logic/guide/24_zh.md#64)的库列表中，选择要导出其数据的库。
3. 点击以下任意一项：
   
    - 封装 — 导出 PCB 封装（元件封装）
    - 元件 — 导出组件线 — 导出绘图对象
    - 逻辑 — 导出 [CAE 封装](/logic/guide/7_zh.md#7)（原理图符号）

4. 要过滤列表，请在筛选条件框中键入通配符或表达式，然后点击应用。

    > [!TIP]
    > 
    > 提示 要显示库中的所有项目，请键入星号 (\*) 并点击“应用”。

5. 在列表中选择一个或多个项目，然后点击“导出”。
6. 在“库导出文件”对话框中，指定文件夹，键入文件名，然后点击“保存”。
...
3. 在“筛选条件”区域中，点击“元件”。库中（或所有库中）的元件列表将显示在“元件类型”区域中。

    > [!TIP]
    >
    > 要优化列表，请使用筛选字段。在字段中输入元件名称，或使用通配符 $(^{\star})$ 指定一组元件。然后点击“应用”。

4. 当您拥有要报告的元件列表时，请点击“列出到文件”。
5. 在报告管理器对话框中，指定要包含在报告中的元件属性。

    a. 在可用属性列表中，点击某个属性以将其选中。

    b. 点击“包括”。

    属性将显示在“选定属性”列表中。

    要从“选定属性”列表中删除属性，请选择它们并点击“<<Exclude”。
...
3. 在筛选条件区域中，点击封装、线或逻辑筛选条件。

    对话框中会出现库中相应项目的列表（在“PCB 封装”、“线项目”或“[CAE 封装](/logic/guide/7_zh.md#7)”区域中，取决于您的筛选条件选择）。

    > [!TIP]
    > 
    > 要选择一个或多个特定项目，请使用筛选字段。在字段中输入项目名称，或使用通配符 `(*)`指定一组项目。然后点击“应用”。

4. 当您有了想要报告的列表时，请点击“列出到文件”。

5. 在“库列表文件”对话框中，指定报告的文件夹和文件名，然后点击“保存”。
...
您可以使用[通配符和表达式](#8)来过滤显示的信息。这有助于快速、准确地浏览库内容。

该筛选条件支持表 13 中列出的[通配符和表达式](#8)。表 14 给出了通配符使用的示例。
