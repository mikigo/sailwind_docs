...
若发现元件存在错误，请提交服务请求至market@pzeda.com，以便对元件进行单独审查并修复或从库中移除。
...
- -24\_20N = 24 管脚封装中的 20 管脚元件。管脚编号为 1 至 20。原理图符号显示 20 管脚。
...
| 直角插头                     | HDRRA + 总管脚数 + W 引线宽度 + P 行间距（+ X 列间距[如果不同]） + _行数 + X 每行管脚数 + _本体长度 + X 本体厚度 + X 元件高度 + 制造等级 |
...
- IPC-7353 = IEC 61188-5-3，分段要求 连接（焊盘/焊点）考虑 鸥翼管脚，两侧（SOP）
...
以下示例焊盘堆叠名称基于 IPC-7251/7351 [焊盘堆叠命名规范](#6)。使用[焊盘堆叠命名规范](#6)的示例（所有值均为公制单位）：
...
**字母 v 的使用**：过孔可以使用[焊盘堆叠命名规范](#6)命名。由于大多数过孔使用圆形焊盘，字母 V 将代替字母 C 在[焊盘堆叠命名规范](#6)中使用。如果不正确，可以在字母 V 后添加修饰符以表示形状或尺寸的更改。

**字母 w 的使用**：除了过孔，[焊盘堆叠命名规范](#6)还可用于定义安装孔。字母 W 用于定义安装孔特性以及用于表面焊盘（镀或非镀）的任何相关焊盘。
