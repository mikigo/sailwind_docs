...
4. 点击**确定**
5. 在"[重新分配电气层](/layout/guide/14_zh.md#15)"对话框中，接受默认的层 1 分配到层 1 和层 2 分配到层 4 的分配，点击**确定**
...
启动文件是一种在设计之间重复使用设计选项的方法。启动文件包含预定义的系统设置，如层数、显示颜色分配、当前工作网格、设计规则和许多其他常用选项。您可以为特定板类型(如双层或四层 PCB)[创建启动文件](/layout/guide/10_zh.md#2)。然后可以将启动文件用于每个 PCB 设计，避免在每个新设计中输入标准选项。
...
2. 右键点击并选择**矩形**
3. 将鼠标移动到 **X200,Y1900** 位置，点击添加板框边框的第一个角。一条动态线附着在鼠标上
4. 使用搜索无模命令将鼠标定位到指定位置。输入 **s 3000 300** (包括空格)并按**Enter**
5. 按**空格键**完成矩形

## 修改板框边框 \{#4}

下一个过程通过将现有直线之一更改为圆弧来[修改板框边框](#4)的一部分。在[修改板框边框](#4)之前，必须调整选择过滤器以定义可选择的对象类型。

1. 右键点击并选择**选择板框边框**以启用板框边框元素的选择
2. 点击板框边框右侧的垂直线以选择它
3. 右键点击并选择**拉出圆弧**
4. 将鼠标向右移动，将圆弧从板框拉出，到 **X3800,Y1100** 位置
5. 点击放置圆弧。
    > [!TIP]
...
2. 点击**确定**确认创建开槽并继续操作
3. 右键点击并选择**矩形**
4. 将鼠标移动到**X2400,Y1700**位置，点击开始矩形
5. 将鼠标移动到**X2500,Y500**位置，点击完成板开槽
...
1. **绘图工具栏** > **禁止区域**按钮
2. 右键点击并选择**矩形**
3. 将鼠标定位在板开槽的左上角(**X2400,Y1700**)，点击开始禁止区域矩形
4. 将鼠标向板的左下角(**X600,Y500**)移动，点击完成禁止区域
...
3. 在 [`ECO` 工具栏](/layout/guide/36_zh.md#2)上，点击**添加元件**按钮
...
6. 选择**SHIELD**元件类型并点击**添加**将屏蔽罩元件添加到设计中。
    ...
    >
    > 元件被添加到设计并附着在鼠标上
7. 元件附着在鼠标上时，点击**关闭**关闭"从库获取元件类型"对话框
8. 将屏蔽罩元件移动到 **X2600,Y600**，点击放置
9. 再次点击 [`ECO` 工具栏](/layout/guide/36_zh.md#2)按钮退出 `ECO` 模式和添加元件
