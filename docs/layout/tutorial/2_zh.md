...
您可以使用库管理器和 [PCB 封装编辑器](/layout/guide/Dialogs/52_zh.md#15)在 SAILWIND 库中创建构成元件类型的库元素。
...
**[PCB 封装编辑器](/layout/guide/Dialogs/52_zh.md#15)**

PCB 封装是元件的物理表示，通常称为*封装*。PCB 封装包含多个代表元件管脚的焊盘和一个元件边框。您可以在 [PCB 封装编辑器](/layout/guide/Dialogs/52_zh.md#15)中[创建 PCB 封装](#2)。

在工具菜单中，点击 [PCB 封装编辑器](/layout/guide/Dialogs/52_zh.md#15)。

进入 [PCB 封装编辑器](/layout/guide/Dialogs/52_zh.md#15)后，会显示文本标签和 PCB 封装原点标记：
...
在以下练习中，您将通过添加新焊盘、元件边框和分配焊盘栈，为下图所示的 26 管脚连接器[创建 PCB 封装](#2)。
...
4. 将鼠标移动到封装原点标记上，点击以在原点添加第一个焊盘。
5. 将鼠标移动到 **X50,Y50** 位置（参考状态栏的 X,Y 坐标显示），点击添加第二个焊盘。
...
3. 输入**s -100 100**（s, 负 100, 空格, 100）并按**Enter**，将鼠标定位到边框的左上角。
4. 不移动鼠标，按**空格键**开始新的矩形多边形。
5. 将鼠标移动到 **X1350,Y -50** 位置。
6. 点击完成多边形。
...
2. 右键点击并选择**矩形**。
3. 将鼠标定位到边框的左上角，点击开始绘制禁止区域矩形。
4. 将鼠标移动到边框的右下角，点击完成多边形。
...
1. **编辑**菜单 > **[属性管理器](/layout/guide/18_zh.md#9)**。
...
4. 将它们放置在教程开头"[创建 PCB 封装](#2)"部分所示的大致位置。
5. 点击完成移动。
...
[创建连接器元件类型](/layout/guide/8_zh.md#7)的第一步是将其定义为连接器并分配 PCB 封装。

1. **主页**菜单 > **库**。
2. 要在 preview 库中[创建新元件类型](#6)，在库列表中点击**…\preview**。
3. 点击**元件**按钮。
4. 点击**新建**。
...
下一步是[为元件类型分配 PCB 封装](/layout/guide/8_zh.md#8-2)。
...
3. 在[库管理器对话框](/layout/guide/Dialogs/51_zh.md#8)中，点击**关闭**。

## 使用封装向导创建 PCB 封装 \{#7}

[PCB 封装编辑器](/layout/guide/Dialogs/52_zh.md#15)提供对话框驱动的 PCB 封装向导。这些向导提供自动化工具，可自动创建标准封装图案。

**创建 SOIC 封装**

现在您已体验了使用 [PCB 封装编辑器](/layout/guide/Dialogs/52_zh.md#15)中的交互工具[创建 PCB 封装](#2)，接下来将使用 [PCB 封装编辑器](/layout/guide/Dialogs/52_zh.md#15)中的自动化工具创建下图所示的 SO28 PCB 封装。
