...
有关创建 CAE 封装的信息，请参阅《`SailWind Logic` 指南》中的"创建新 CAE 封装"。有关创建线图图形的信息，请参阅本手册中的"[从库中添加绘图项目](/layout/guide/27_zh.md#15-10)"。
...
> !tip] 您也可以添加现有库 - 更多信息请参阅"[[向库列表添加库](#6-1)"。
...
您的库被添加到库列表底部，这也是搜索顺序中的最后一个库。要将库在库列表和搜索顺序中上移，请参阅"[设置库列表顺序](#6-4)"。

## 显示库中的项目 \{#4}

使用[库管理器对话框](/layout/guide/Dialogs/51_zh.md#8)显示库中包含的项目。

🏃‍♂️‍**步骤**

1. 点击**主页>库**菜单项

2. 在[库管理器对话框](/layout/guide/Dialogs/51_zh.md#8)中，从库列表中选择一个库或选择列表顶部的(所有库)项

3. 点击以下按钮之一[显示库中的项目](#4)类别：
...
4. 空筛选条件框不会产生结果。要在筛选条件框中输入通配符或表达式来过滤列表。如果不想用筛选条件限制结果，可以在筛选条件框中输入*(星号)显示所有项目
...
`SailWind Layout` 提供了多种选项来[修改库数据](#5)以满足您的设计要求。

### 向库中添加项目 \{#5-1}

您可以使用[库管理器对话框](/layout/guide/Dialogs/51_zh.md#8)向库中添加新项目。

🏃‍♂️‍**步骤**

1. 点击**主页>库**菜单项

2. 在[库管理器对话框](/layout/guide/Dialogs/51_zh.md#8)中，从库列表中选择一个库。如果选择(所有库)，**新建**按钮不可用
...
4. 点击**新建**
   ...
   - **封装** 打开 [PCB 封装编辑器](/layout/guide/Dialogs/52_zh.md#15)，使用[创建新封装](/layout/guide/7_zh.md#4)所需的默认配置。更多信息请参阅"[创建新封装](/layout/guide/7_zh.md#4)"

   - **元件** 打开元件信息对话框，使用创建元件类型所需的默认配置。更多信息请参阅"[创建新元件类型](/layout/tutorial/2_zh.md#6)"

   - **线图** 新建和编辑不可用。没有专门的库线图编辑器。使用绘图工具创建或编辑线图并将其保存到库中。更多信息请参阅"[创建绘图对象](/layout/guide/27_zh.md#1)"和"将绘图项目保存到库中"
...
2. 在[库管理器对话框](/layout/guide/Dialogs/51_zh.md#8)中，从库列表中选择一个库。如果选择(所有库)，**删除**按钮不可用
...
2. 在[库管理器对话框](/layout/guide/Dialogs/51_zh.md#8)中，从库列表中选择一个库
...
6. 点击**编辑**
   ...
   - **封装** 打开 [PCB 封装编辑器](/layout/guide/Dialogs/52_zh.md#15)并显示选定的封装。更多信息请参阅"[编辑库封装](/layout/guide/7_zh.md#5-2)"

   - **元件** 打开元件信息对话框并显示选定的元件。更多信息请参阅"创建和[修改元件类型](/layout/guide/8_zh.md#8)"

   - **线图** 不可用。没有专门的库线图编辑器。使用绘图工具创建或编辑线图并将其保存到库中。更多信息请参阅"[创建绘图对象](/layout/guide/27_zh.md#1)"和"将绘图项目保存到库中"
...
> !tip] 要复制单个项目，请参阅"[[复制库项目](#5-3)"。

1. 按照鼠标步骤从库中[导出库数据](#8-2)

2. 然后按照鼠标步骤将库数据导入到另一个库

## 库可用性与搜索选项 \{#6}

使用[库列表对话框](/layout/guide/Dialogs/51_zh.md#7)指定与搜索相关的选项，如库搜索顺序和哪些库可供设计使用。此对话框中的操作会影响[库管理器对话框](/layout/guide/Dialogs/51_zh.md#8)中库列表的内容。
...
2. 在[库列表对话框](/layout/guide/Dialogs/51_zh.md#7)中，点击**添加**
...
2. 在[库列表对话框](/layout/guide/Dialogs/51_zh.md#7)中，在库列表中选择一个或多个库，然后点击**移除**
...
- [从库更新设计](/layout/guide/7_zh.md#8)

- 用原理图中的新元件注释设计

您可以使用[库列表对话框](/layout/guide/Dialogs/51_zh.md#7)更改库的搜索顺序。
...
2. 在[库列表对话框](/layout/guide/Dialogs/51_zh.md#7)中，在库列表中选择库，然后根据需要点击**上移**或**下移**
...
2. 在[库列表对话框](/layout/guide/Dialogs/51_zh.md#7)中，在库列表中选择库。您可以使用 Shift 和 Ctrl 键选择多个库
...
[库列表对话框](/layout/guide/Dialogs/51_zh.md#7)中的库"只读"复选框在您重新打开对话框之前不会更新。
...
2. 在[库列表对话框](/layout/guide/Dialogs/51_zh.md#7)中，选中"与 `SailWind Logic` 同步"复选框
...
如果您正在使用的库是只读的，此过程将不会产生结果或警告。在[库列表对话框](/layout/guide/Dialogs/51_zh.md#7)中检查库的状态。

🏃‍♂️‍**步骤**

1. 点击**主页>库**菜单项

2. 在[库管理器对话框](/layout/guide/Dialogs/51_zh.md#8)中，点击**[属性管理器](/layout/guide/18_zh.md#9)**
...
使用[库管理器对话框](/layout/guide/Dialogs/51_zh.md#8)以 ASCII 格式导入或[导出库数据](#8-2)。

### 导入库数据 \{#8-1}

您可以从先前导出的库 ASCII 文件[导入库数据](#8-1)。

> !tip] 从 PADS 9.0 开始，芯片元件和倒装芯片不再由其系列名称(DIE 或 FLP)标识，而是由[元件信息对话框的"常规"选项卡中的"特殊用途"设置标识。当您导入由早期 PADS 版本创建的 ASCII 文件时，这些"特殊用途"设置会自动设置为具有逻辑系列 DIE 或 FLP 的元件。元件的系列名称保持不变。

🏃‍♂️‍**步骤**

1. 点击**主页>库**菜单项

2. 在[库管理器对话框](/layout/guide/Dialogs/51_zh.md#8)中，在库列表中，选择要接收库数据的库
...
2. 在[库管理器对话框](/layout/guide/Dialogs/51_zh.md#8)中，在库列表中，选择要导出其数据的库
...
2. 在[库管理器对话框](/layout/guide/Dialogs/51_zh.md#8)中，从库列表中选择一个库或选择(所有库)
...
更多信息，请参阅"[创建库中元件报告](#9-1)"。

🏃‍♂️‍**步骤**

1. 点击**主页>库**菜单项

2. 在[库管理器对话框](/layout/guide/Dialogs/51_zh.md#8)中，从库列表中选择一个库。如果选择"所有库"，对于"封装"、"线图"和"逻辑符号"，"列出到文件"按钮不可用
