...
2. 在[设置启动文件对话框](/layout/guide/Dialogs/54_zh.md#22)中，选择启动文件。启动文件包含全局设置，如层定义、网格、间距规则、属性辞典等。
...
1. 通过手动导入 SailWind Logic 网表创建新 PCB 设计

2. [从 OrCAD 网表创建新 PCB 设计](#4)

## 交叉探测 ಠ_ಠ \{#3}

您可以使用 `SailWind Logic` 进行交叉探测，以简化网表导入和元件放置。

交叉探测的步骤取决于您使用的原理图工具。具体产品说明请参考*`SailWind Logic` 指南*中的"SailWind 产品间交叉探测"。
...
4. 在[报告对话框](/layout/guide/Dialogs/53_zh.md#6)中选择"PowerPCB V3.0 格式网表"，点击**确定**。
...
11. 如果产生*ascii.err*文件，请执行以下步骤：
    ...
    b. 将*ascii.err*文件中列出的缺失组件添加到库中，通过添加包含缺失组件的库到库列表，或在现有库中创建缺失的元件类型和封装（有关如何执行这些任务，请参考"添加库到库列表"、"创建和[修改元件类型](/layout/guide/8_zh.md#8)"和"创建和编辑 PCB 封装"）。
...
如果设计简单，且可以跳过原理图创建，您可以从 `SailWind Layout` 中的 [`ECO` 工具栏](/layout/guide/36_zh.md#2)添加元件、创建连接和创建约束。[`ECO` 工具栏](/layout/guide/36_zh.md#2)包含所有设计修改工具。

如果设计是原理图驱动的，它必须在原理图和布局工具间保持同步。您可以将 [`ECO` 工具栏](/layout/guide/36_zh.md#2)工具的设计修改记录在 `ECO` 文件中，用于同步原理图设计和布局设计。

如果创建的是布局驱动设计，且没有原理图需要同步，则无需维护*.eco*文件。当打开 [`ECO` 工具栏](/layout/guide/36_zh.md#2)时，可以取消勾选"写入 `ECO` 文件"复选框，然后忽略 [`ECO` 选项对话框](/layout/guide/Dialogs/49_zh.md#1)的所有设置。

使用 [`ECO` 工具栏](/layout/guide/36_zh.md#2)上的工具，您可以：
...
有关 [`ECO` 工具栏](/layout/guide/36_zh.md#2)的更多信息，请参考"[`ECO` 模式操作（布局驱动设计工具）](/layout/guide/36_zh.md#7)"。